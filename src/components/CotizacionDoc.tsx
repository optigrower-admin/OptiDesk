  ...
  return (
    <>
      <style>{`
        ...
        /* Pantalla: 216mm de ancho */
        .cot-doc { width: 216mm; }

        /* Impresión: zoom 0.9 → menos reducción → letra más grande.
           216 / 0.9 = 240mm de ancho real → visual = 216mm = carta. */
        @media print {
        ...
        <div className="cot-doc" style={{ background: '#fff', boxShadow: '0 8px 60px rgba(0,0,0,0.2)', fontFamily: "'Segoe UI', Arial, sans-serif", fontSize: 12 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #e0eeff 0%, #dbeafe 45%, #eff6ff 100%)', display: 'grid', gridTemplateColumns: '1fr 185px', alignItems: 'stretch', borderBottom: '2px solid #93c5fd' }}>
            ...
            <div style={{ padding: '14px 20px 12px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 6 }}>
              ...
              {tenant.tagline && (
                <div style={{ color: '#64748b', fontSize: 8, letterSpacing: 2, textTransform: 'uppercase', marginTop: -2 }}>{tenant.tagline}</div>
              )}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, paddingTop: 8, borderTop: '1px solid #93c5fd', marginTop: 'auto' }}>
                {[
                  ...
                  <div key={c.text} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    ...
                    <span style={{ fontSize: 7.5, color: '#1d4ed8' }}>{c.text}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ borderLeft: '1px solid #93c5fd', padding: '14px 16px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', gap: 6 }}>
              <div style={{ color: '#64748b', fontSize: 8, textTransform: 'uppercase', letterSpacing: 2 }}>Cotización</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 2 }}>
                <div style={{ color: '#1d4ed8', fontSize: 9.5 }}>📅 {formatFecha(cotizacion.fecha_generacion)}</div>
                <div style={{ color: '#64748b', fontSize: 9 }}>⏰ Válida {cotizacion.vigencia_dias} días</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '64% 36%', alignItems: 'start' }}>
            ...
            <div style={{ padding: '13px 16px 12px', borderRight: '1px solid #e8edf5' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 7, marginBottom: 11 }}>
                    {allPhotos.map((f, i) => (
                      <div key={i} style={{ background: 'linear-gradient(135deg, #f0f5ff, #e8f0fe)', borderRadius: 9, padding: 7, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, minHeight: 115 }}>
                        ...
                        {f.label && <div style={{ fontSize: 7, color: '#64748b', fontWeight: 600, textAlign: 'center' }}>{f.label}</div>}
                      </div>
                    ))}
                  </div>
                )
              ...
              <div style={{ background: '#eff6ff', borderRadius: 8, padding: '8px 11px', marginBottom: 10, border: '1.5px solid #bfdbfe' }}>
                <div style={{ fontSize: 8, fontWeight: 800, color: '#1d4ed8', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 7 }}>Esta cotización incluye</div>
              ...
              {specsItems.length > 0 && (
                <div style={{ marginBottom: 10, background: '#f8faff', borderRadius: 8, padding: '8px 11px', border: '1px solid #e2e8f0' }}>
                  <div style={{ fontSize: 8, fontWeight: 800, color: '#0052B4', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 7, paddingBottom: 4, borderBottom: '1.5px solid #dde3f0' }}>
              ...
              <div style={{ background: '#eff6ff', borderRadius: 8, padding: '8px 11px', border: '1px dashed #93c5fd' }}>
                <div style={{ fontSize: 7.5, fontWeight: 800, color: '#1d4ed8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4 }}>📝 Notas adicionales</div>
                {cotizacion.notas
                  ? <div style={{ fontSize: 8, color: '#1e3a8a', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{cotizacion.notas}</div>
            ...
            <div style={{ padding: '13px 12px', background: '#f8faff', display: 'flex', flexDirection: 'column', gap: 9 }}>
              ...
              {(verContado || verPignorada) && (
                <div style={{ background: '#eff6ff', border: '2px solid #0052B4', borderRadius: 10, padding: '11px 13px' }}>
                  <div style={{ fontWeight: 800, fontSize: 9.5, color: '#0052B4', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 9, borderBottom: '1.5px solid #bfdbfe', paddingBottom: 5 }}>
                    Precio
                  </div>
                  {verContado && (
                    <div style={{ marginBottom: verPignorada ? 9 : 0 }}>
                      <div style={{ fontSize: 8, color: '#1d4ed8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 3 }}>Contado con papeles</div>
                      <div style={{ fontSize: 23, fontWeight: 900, color: '#1e3a8a', lineHeight: 1, letterSpacing: -0.5 }}>{cop(conPapeles)}</div>
                      <div style={{ fontSize: 7.5, color: '#64748b', marginTop: 2 }}>IVA incluido · SOAT · Matrícula</div>
                    </div>
                  )}
                  {verPignorada && (
                    <div style={{ borderTop: verContado ? '1.5px solid #bfdbfe' : 'none', paddingTop: verContado ? 9 : 0 }}>
                      <div style={{ fontSize: 8, color: '#1d4ed8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 3 }}>En crédito (pignorada)</div>
                      <div style={{ fontSize: verContado ? 18 : 23, fontWeight: 900, color: '#0052B4', lineHeight: 1 }}>{cop(pignorada)}</div>
                      <div style={{ fontSize: 7.5, color: '#64748b', marginTop: 2 }}>Incluye prenda · Con papeles</div>
                    </div>
                  )}
                </div>
              )}
              ...
              {(() => {
                ...
                <div style={{ background: '#eff6ff', border: '2px solid #0052B4', borderRadius: 10, padding: '13px 12px' }}>
                  ...
                  <div style={{ textAlign: 'center', marginBottom: 10 }}>
                    <div style={{ fontWeight: 900, fontSize: 13, color: '#1e3a8a', letterSpacing: -0.3 }}>¡APARTA HOY!</div>
                    <div style={{ fontSize: 7.5, color: '#1d4ed8', marginTop: 3, lineHeight: 1.4 }}>
                      No pierdas esta oportunidad.
                    </div>
                  </div>

                  {/* Título contacto */}
                  <div style={{ fontSize: 7.5, fontWeight: 800, color: '#0052B4', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 7 }}>
                    ...
                    <div style={{ borderTop: '1.5px solid #bfdbfe', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 8 }}>
                      {tenant.telefono1 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ fontSize: 9.5, color: '#1e3a8a', fontWeight: 700 }}>{fmtPhone(tenant.telefono1)}{tenant.telefono2 ? `  ·  ${fmtPhone(tenant.telefono2)}` : ''}</span>
                        </div>
                      )}
                      {tenant.email && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ fontSize: 9.5, color: '#1e3a8a', fontWeight: 700 }}>{tenant.email}</span>
                        </div>
                      )}
                      {tenant.web && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ fontSize: 9.5, color: '#1e3a8a', fontWeight: 700 }}>{tenant.web}</span>
                        </div>
                      )}
                      {tenant.direccion && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ fontSize: 9.5, color: '#1e3a8a', fontWeight: 700 }}>{tenant.direccion}</span>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {!tenant.whatsapp && !tenant.instagram && !tenant.facebook && !tenant.tiktok && !tenant.telefono1 && !tenant.email && !tenant.direccion && (
                    <div style={{ fontSize: 7.5, color: '#64748b', fontStyle: 'italic', marginBottom: 8 }}>
                      Completa tus datos en Config. Ventas.
                    </div>
                  )}

                  <div style={{ borderTop: '1.5px solid #bfdbfe', paddingTop: 9, textAlign: 'center', fontFamily: 'Georgia, serif', fontStyle: 'italic', color: '#1e3a8a', fontSize: 10, lineHeight: 1.5 }}>
              ...
              {(() => {
                ...
                return (
                  <div style={{ background: '#fff', border: '1.5px solid #dde3f0', borderRadius: 8, padding: '8px 10px' }}>
                    ...
                    <div style={{ fontSize: 7.5, color: '#334155', fontStyle: 'italic', lineHeight: 1.5 }}>{texto}</div>
                    <div style={{ marginTop: 5, display: 'flex', alignItems: 'center', gap: 5 }}>
                      <div style={{ width: 19, height: 19, borderRadius: '50%', background: '#0052B4', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 8, fontWeight: 800, flexShrink: 0 }}>{inicial}</div>
                      <div>
                        <div style={{ fontSize: 7, fontWeight: 700, color: '#1e293b' }}>{autor}</div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #e0eeff 0%, #dbeafe 50%, #eff6ff 100%)', borderTop: '2px solid #93c5fd', padding: '10px 24px', display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 12 }}>
            <div>
              <div style={{ color: '#64748b', fontSize: 7.5, textTransform: 'uppercase', letterSpacing: 2, marginBottom: 1 }}>Tu próxima aventura</div>
              <div style={{ color: '#1e3a8a', fontSize: 15, fontWeight: 900, letterSpacing: -0.5, textTransform: 'uppercase', lineHeight: 1 }}>¡COMIENZA AQUÍ!</div>
            ...
            <div style={{ textAlign: 'center', maxWidth: 150 }}>
              <div style={{ fontSize: 7.5, color: '#1d4ed8', lineHeight: 1.5 }}>
