...
// El correo sale SIEMPRE desde la cuenta de Gmail de la EMPRESA (una sola, conectada
// una vez en Bot Colaboradores → Correo de la empresa) hacia el correo de
// notificación de cada asesor — nadie más necesita conectar nada.
...
export async function ejecutarResumenDiario(
  ...
  for (const [usuarioId, items] of porUsuario) {
    ...
    if (enviarEmail) {
      try {
        await sendEmailComoEmpresa(
          usuario.tenant_id, usuario.email,
