...
export async function POST(req: NextRequest) {
  ...
  const { moto_catalogo_id, index, content_type } = await req.json() as {
    moto_catalogo_id: string
    index: number
    content_type: string
  }

  if (!moto_catalogo_id || index === undefined) {
    return NextResponse.json({ error: 'Parámetros inválidos' }, { status: 400 })
  }

  const ext = content_type?.includes('png') ? 'png' : content_type?.includes('webp') ? 'webp' : 'jpg'
  const key = `${perfil.tenant_id}/catalogo/${moto_catalogo_id}/color_${index}.${ext}`
