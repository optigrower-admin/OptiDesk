...
export default function PopupSelectPage() {
  ...
  useEffect(() => {
    ...
    try {
      const p = JSON.parse(decodeURIComponent(params.get('phones') ?? '[]'))
      setPhones(p)
      setSelected(p[0] ?? null)
    } catch { setError('Error al cargar los números') }
    setWabaId(params.get('waba_id') ?? '')
    setWabaName(decodeURIComponent(params.get('waba_name') ?? ''))
      ...
      <div className="max-w-sm w-full">
        <div className="mb-6 text-center">
          <div className="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center mx-auto mb-3">
            <svg viewBox="0 0 24 24" className="w-7 h-7 text-green-700" fill="currentColor">
          ...
          <h2 className="font-bold text-gray-900 text-lg">Selecciona el número</h2>
          {wabaName && <p className="text-sm text-gray-500 mt-0.5">{wabaName}</p>}
        </div>
        ...
        <div className="space-y-2 mb-6">
          {phones.map(phone => (
            <label key={phone.id} className={`flex items-center gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${selected?.id === phone.id ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300'}`}>
              <input type="radio" name="phone" checked={selected?.id === phone.id}
                onChange={() => setSelected(phone)} className="accent-green-600" />
              <div>
