...
export default function RecordatoriosTab({ clienteId, tenantId, usuarioId, clienteEmail, onProximaAccionChange }: Props) {
  ...
  async function descompletar() {
    ...
    await supabase.from('recordatorios').update({ completado: false, completado_at: null }).eq('id', confirmUncheckId)
    await sincronizarProximaAccion()
    ...
    <div className="space-y-4">
      ...
      <div className="border-t pt-3">
        ...
        {pendientes.map(r => {
          ...
          return (
            <div key={r.id} className={`border rounded-xl px-3 py-2 mb-2 ${vencido ? 'bg-red-50 border-red-300' : 'bg-blue-50 border-blue-200'}`}>
          ...
          {pasados.map(r => (
            <div key={r.id}>
              <div className={`border rounded-xl px-3 py-2 ${
                confirmUncheckId === r.id
                  ? 'bg-red-50 border-red-200 rounded-b-none mb-0'
                  : 'bg-green-50 border-green-200 mb-2'
              }`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                  ...
                  {confirmUncheckId !== r.id && (
                    <button
                      onClick={() => setConfirmUncheckId(r.id)}
                      className="flex-shrink-0 text-xs text-gray-400 hover:text-red-500 transition-colors border border-gray-200 hover:border-red-300 rounded-lg px-2 py-0.5"
              ...
              {confirmUncheckId === r.id && (
                <div className="bg-red-50 border border-red-200 border-t-0 rounded-b-xl px-3 py-2 mb-2 text-xs text-red-700">
